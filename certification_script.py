...
"""
Основной скрипт проекта для запуска из консоли, как аргументы принимает:
1. Путь к директории с xlsx файлами, в которых содержатся оценки (Оценка_DE_сам.оценка+TL оценка.xlsx).
2. Путь к xlsx файлу в котором содержатся целевые значения для проведения сертификации (Компетенции_по_шкале_DE.xlsx)
3. Путь к директории, в которую будут сохранятся результирующие xlsx файлы по одному на каждый файл с оценками.
"""
    ...
    try:
        ...
        for key, val in test_result.items():
            if val[0]:  # Пока уровень достигнут переходим дальше.
                ...
                compliance = val[1]
            else:  # Как только попали на False, запоминаем этот уровень (первый еще не достигнутый).
...
if __name__ == '__main__':
    if len(sys.argv) != 4:
        print("Как использовать: \путь\к\certification_script.py \путь\к\директории с файлами "
              "\путь\к\Компетенции_по_шкале_DE.xlsx путь\к\куда сохранить")
        sys.exit(1)
    directory_path = sys.argv[1]
    target_values_file = sys.argv[2]
    save_to = sys.argv[3]
    marks_file = []
    for filename in os.listdir(directory_path):
        if filename.endswith(".xlsx"):
            marks_file.append(os.path.join(directory_path, filename))
    for path in marks_file:
        res = certificate(path, 'Оценка компетенций', target_values_file, 'Целевые значения', save_to)
